...
export const AdminPortal: React.FC = () => {
  ...
  const handleSaveClass = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingClass.id) {
      db.updateClass(editingClass as ClassType);
    } else {
      const newClass: ClassType = {
        id: `c${Date.now()}`,
        name: editingClass.name || 'New Class',
        description: editingClass.description || '',
        priceSingle: Number(editingClass.priceSingle) || 0,
        pricePackage: Number(editingClass.pricePackage) || 0,
        durationMinutes: Number(editingClass.durationMinutes) || 45,
        difficulty: editingClass.difficulty || 'Beginner'
      };
  ...
  const handleSavePackage = (e: React.FormEvent) => {
    ...
    if (editingPackage.id) {
      db.updatePackage(editingPackage as Package);
    } else {
      const newPkg: Package = {
        id: `p${Date.now()}`,
        name: editingPackage.name || 'New Package',
        credits: Number(editingPackage.credits) || 5,
        price: Number(editingPackage.price) || 100,
  ...
  const handleSaveSettings = () => {
    db.saveSettings(systemSettings);
                        ...
                        <div>
                          <div className="font-bold text-zinc-900 flex items-center gap-2">
                            ...
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${
                              user.role === UserRole.ADMIN ? 'bg-zinc-900 text-white' : 
                         ...
                         <div className="flex-1">
                           <div className="flex items-center justify-between mb-3">
                              ...
                              <span className={`text-[10px] px-2 py-1 rounded font-bold uppercase tracking-wide border ${
                                 cls.difficulty === 'Beginner' ? 'border-zinc-400 text-zinc-500' : 

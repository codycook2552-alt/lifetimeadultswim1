...
import { User, LessonSession, ClassType, Purchase } from '../types';
...
export const ClientPortal: React.FC<ClientPortalProps> = ({ user, onBookClick }) => {
  const [upcomingSessions, setUpcomingSessions] = useState<LessonSession[]>([]);
  const [pastSessions, setPastSessions] = useState<LessonSession[]>([]);
  const [classTypes, setClassTypes] = useState<ClassType[]>([]);
  const [purchases, setPurchases] = useState<Purchase[]>([]);

  useEffect(() => {
    setClassTypes(db.getClasses());
    const allUserSessions = db.getSessionsForUser(user.id);
    const now = new Date();
    
    setUpcomingSessions(allUserSessions.filter(s => new Date(s.startTime) > now).sort((a,b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime()));
    setPastSessions(allUserSessions.filter(s => new Date(s.startTime) <= now).sort((a,b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()));
    setPurchases(db.getPurchasesForUser(user.id));
  }, [user.id]);

  const getClassName = (id: string) => classTypes.find(c => c.id === id)?.name || 'Swim Lesson';
